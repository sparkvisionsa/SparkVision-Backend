...
exports.getCompanyReportDefaultsForCompanyAdmin = getCompanyReportDefaultsForCompanyAdmin;
exports.updateCompanyReportDefaultsByCompanyAdmin = updateCompanyReportDefaultsByCompanyAdmin;
...
exports.getCurrentCompanyUserSignature = getCurrentCompanyUserSignature;
exports.createCompanyReportOnlySignatory = createCompanyReportOnlySignatory;
exports.updateCompanyReportOnlySignatory = updateCompanyReportOnlySignatory;
exports.updateCompanyReportOnlySignatorySignature = updateCompanyReportOnlySignatorySignature;
exports.deleteCompanyReportOnlySignatory = deleteCompanyReportOnlySignatory;
...
const company_report_defaults_constants_1 = require("./company-report-defaults.constants");
...
const VALUE_TECH_PRODUCT_ID_SET = new Set(VALUE_TECH_PRODUCT_IDS);
function normalizeValueTechProductId(value) {
    if (typeof value !== "string")
        return null;
    const trimmed = value.trim();
    return VALUE_TECH_PRODUCT_ID_SET.has(trimmed) ? trimmed : null;
}
...
function readQueryString(request, key) {
    const value = request.query?.[key];
    if (Array.isArray(value))
        return typeof value[0] === "string" ? value[0] : undefined;
    return typeof value === "string" ? value : undefined;
}
function readProductIdFromRequest(request) {
    return normalizeValueTechProductId(readQueryString(request, "productId"));
}
function membershipProductIds(membership, company) {
    const companyProductIds = normalizeProductIds(company?.valueTechProductIds);
    if (!membership)
        return companyProductIds;
    if (membership.role === "company_admin")
        return companyProductIds;
    const explicit = normalizeProductIds(membership.productIds);
...
function membershipHasProductAccess(membership, productId, company) {
    if (membership.role === "company_admin")
        return true;
    return membershipProductIds(membership, company).includes(productId);
}
function mergeProductIds(current, productId, company) {
...
function requirePhoneIdentifier(value, message = "Phone number must include country code.") {
...
async function assertPhoneIdentifierAvailable(users, phone, excludeUserId) {
...
async function findUserByLoginIdentifier(users, value) {
...
const loginSchema = zod_1.z.object({
    ...
    phone: zod_1.z.string().trim().min(3).max(64).optional(),
...
const profileSchema = zod_1.z.object({
    ...
    valuationReportJobTitle: zod_1.z.string().trim().max(160).optional().nullable(),
    valuationReportMembershipNo: zod_1.z.string().trim().max(80).optional().nullable(),
...
function resolveValueTechProductIds(user, company, membership) {
...
function sanitizeReportDefaultsText(value, max) {
    if (typeof value !== "string")
        return "";
...
function isReportDefaultsImageReference(value) {
    return (value === "" ||
        value.startsWith("data:image/") ||
        value.startsWith(REPORT_DEFAULTS_LETTERHEAD_UPLOAD_PREFIX));
}
function sanitizeReportDefaultsImageReference(value) {
    if (typeof value !== "string")
        return null;
...
function isReportDefaultsWordTemplateUrl(value) {
...
function parseReportDefaultsImageDataUrl(value) {
...
async function persistCompanyReportLetterheadImage(companyId, field, value) {
    if (typeof value !== "string")
        return null;
    const normalized = value.replace(/\u0000/g, "").trim();
    if (!normalized)
    ...
    if (!parsed) {
        throw new HttpError(400, "invalid_payload", "Report letterhead image must be PNG, JPG, or WEBP and no larger than 8MB after compression.");
    }
    const safeCompanyId = companyId.replace(/[^a-zA-Z0-9_-]/g, "");
    const safeField = field.replace(/[^a-zA-Z0-9_-]/g, "-").slice(0, 120) || "image";
    const dir = (0, node_path_1.join)(process.cwd(), "uploads", "company-report-templates", safeCompanyId);
    await (0, promises_1.mkdir)(dir, { recursive: true });
    const filename = `${safeField}-${Date.now()}-${(0, crypto_1.randomId)()}.${parsed.ext}`;
...
async function persistCompanyReportWordTemplate(companyId, value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return null;
    const data = value;
    const fileName = sanitizeReportDefaultsText(data.fileName, 240).replace(/[\\/:*?"<>|]+/g, "-") || "word-template.docx";
...
async function persistCompanyReportDefaultsAssets(raw, companyId) {
    const data = raw && typeof raw === "object" && !Array.isArray(raw)
        ? { ...raw }
        : {};
    const letterheadRaw = data.letterhead && typeof data.letterhead === "object" && !Array.isArray(data.letterhead)
        ? { ...data.letterhead }
        : null;
    if (letterheadRaw) {
        const letterhead = { ...letterheadRaw };
    ...
    if (Array.isArray(data.aiTemplates)) {
        data.aiTemplates = await Promise.all(data.aiTemplates.slice(0, 20).map(async (template, index) => {
            const templateRaw = template && typeof template === "object" && !Array.isArray(template)
                ? { ...template }
                : {};
            for (const field of REPORT_DEFAULTS_AI_TEMPLATE_IMAGE_FIELDS) {
                templateRaw[field] = await persistCompanyReportLetterheadImage(companyId, `ai-${index + 1}-${field}`, templateRaw[field]);
            }
            return templateRaw;
        }));
    }
...
function sanitizeCompanyReportCustomGroups(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .slice(0, 30)
...
function sanitizeCompanyReportCustomSections(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .slice(0, 40)
...
function sanitizeCompanyReportLetterheadTemplate(value) {
...
function sanitizeCompanyReportWordTemplate(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return null;
    const data = value;
...
function sanitizeCompanyAiTemplateStringList(value, maxItems, maxChars) {
    if (!Array.isArray(value))
        return [];
    const out = [];
...
function sanitizeCompanyAiReportTemplates(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .slice(0, 20)
        ...
        const sections = Array.isArray(data.sections)
            ? data.sections.slice(0, 60).map((section, sectionIndex) => {
                ...
                return {
                    ...
                    order: typeof sectionData.order === "number" && Number.isFinite(sectionData.order)
        ...
        const dynamicVariables = Array.isArray(data.dynamicVariables)
            ? data.dynamicVariables.slice(0, 120).map((variable, variableIndex) => {
                const variableData = variable && typeof variable === "object" && !Array.isArray(variable)
                    ? variable
                    : {};
                return {
            ...
            : [];
        if (!name && sections.length === 0 && dynamicVariables.length === 0)
            return null;
        return {
...
function sanitizeCompanyReportDefaults(raw) {
    const data = raw && typeof raw === "object" ? raw : {};
    const scopeRaw = data.scope && typeof data.scope === "object" ? data.scope : {};
    const methodologyRaw = data.methodology && typeof data.methodology === "object"
        ? data.methodology
        : {};
    const assumptionsRaw = data.assumptions && typeof data.assumptions === "object"
        ? data.assumptions
        : {};
    return {
        scope: {
...
function shouldUseProOptionBundledWordTemplate(context) {
    ...
    const looksLikeProOption = companyName.includes("prooption") ||
...
function resolveCompanyReportDefaults(stored, context) {
    const seeds = (0, company_report_defaults_constants_1.buildDefaultCompanyReportDefaults)();
    ...
    return {
        scope: {
            ...
            independenceStatement: scopeStored.independenceStatement?.trim() || seeds.scope?.independenceStatement || "",
            intendedUseStatement: scopeStored.intendedUseStatement?.trim() || seeds.scope?.intendedUseStatement || "",
            scopeOfWorkDetails: scopeStored.scopeOfWorkDetails?.trim() || seeds.scope?.scopeOfWorkDetails || "",
            valuationBasisDefinition: scopeStored.valuationBasisDefinition?.trim() || seeds.scope?.valuationBasisDefinition || "",
            valuePremiseDefinition: scopeStored.valuePremiseDefinition?.trim() || seeds.scope?.valuePremiseDefinition || "",
            useRestriction: scopeStored.useRestriction?.trim() || seeds.scope?.useRestriction || "",
            externalSpecialistUse: scopeStored.externalSpecialistUse?.trim() || seeds.scope?.externalSpecialistUse || "",
            esgConsiderations: scopeStored.esgConsiderations?.trim() || seeds.scope?.esgConsiderations || "",
            informationSources: scopeStored.informationSources?.trim() || seeds.scope?.informationSources || "",
        },
        methodology: {
            assetSubjectDescription: methodologyStored.assetSubjectDescription?.trim() ||
                seeds.methodology?.assetSubjectDescription ||
                "",
            assetDetailedDescription: methodologyStored.assetDetailedDescription?.trim() ||
                seeds.methodology?.assetDetailedDescription ||
                "",
            methodologyRationale: methodologyStored.methodologyRationale?.trim() ||
                seeds.methodology?.methodologyRationale ||
                "",
            costApproachDetails: methodologyStored.costApproachDetails?.trim() ||
                seeds.methodology?.costApproachDetails ||
                "",
            salvageValueDescription: methodologyStored.salvageValueDescription?.trim() ||
                seeds.methodology?.salvageValueDescription ||
                "",
            physicalDepreciationDescription: methodologyStored.physicalDepreciationDescription?.trim() ||
                seeds.methodology?.physicalDepreciationDescription ||
                "",
            functionalObsolescenceDescription: methodologyStored.functionalObsolescenceDescription?.trim() ||
                seeds.methodology?.functionalObsolescenceDescription ||
                "",
            economicObsolescenceDescription: methodologyStored.economicObsolescenceDescription?.trim() ||
                seeds.methodology?.economicObsolescenceDescription ||
                "",
        },
        assumptions: {
            generalAssumptions: assumptionsStored.generalAssumptions?.trim() ||
                seeds.assumptions?.generalAssumptions ||
                "",
            specialAssumptions: assumptionsStored.specialAssumptions?.trim() ||
...
const reportDefaultsImageDataUrlSchema = zod_1.z
    .union([
    zod_1.z.string().max(REPORT_DEFAULTS_IMAGE_DATA_URL_MAX_CHARS).refine(isReportDefaultsImageReference, {
        message: "image must be a data URL or a persisted report template upload URL",
    }),
    zod_1.z.null(),
])
    .optional();
const aiTemplateStringListSchema = zod_1.z.array(zod_1.z.string().max(1000)).max(120);
const aiTemplateSchema = zod_1.z.object({
    id: zod_1.z.string().max(120).optional(),
    type: zod_1.z.literal("AI Template").optional(),
    name: zod_1.z.string().max(160).optional(),
    sourceFileName: zod_1.z.string().max(240).optional(),
    createdAt: zod_1.z.string().max(40).optional(),
    updatedAt: zod_1.z.string().max(40).optional(),
    analysisSummary: zod_1.z.string().max(2000).optional(),
    ...
    theme: zod_1.z.record(zod_1.z.unknown()).optional(),
    layout: zod_1.z.record(zod_1.z.unknown()).optional(),
    sections: zod_1.z
        .array(zod_1.z.object({
        id: zod_1.z.string().max(120).optional(),
        title: zod_1.z.string().max(220).optional(),
        order: zod_1.z.number().optional(),
        description: zod_1.z.string().max(1000).optional(),
        dynamicVariables: zod_1.z.array(zod_1.z.string().max(120)).max(80).optional(),
    }))
        .max(60)
        .optional(),
    dynamicVariables: zod_1.z
        .array(zod_1.z.object({
        key: zod_1.z.string().max(120).optional(),
        label: zod_1.z.string().max(220).optional(),
        source: zod_1.z.string().max(220).optional(),
        required: zod_1.z.boolean().optional(),
    }))
        .max(120)
        .optional(),
    rules: aiTemplateStringListSchema.optional(),
    templateJson: zod_1.z.record(zod_1.z.unknown()).optional(),
});
const wordTemplateSchema = zod_1.z
    .object({
    fileName: zod_1.z.string().max(240).optional(),
    fileUrl: zod_1.z
        .union([
        zod_1.z.string().max(REPORT_DEFAULTS_IMAGE_URL_MAX_CHARS).refine(isReportDefaultsWordTemplateUrl, {
    ...
    fileDataUrl: zod_1.z
        ...
        .optional(),
    uploadedAt: zod_1.z.string().max(40).optional(),
    sizeBytes: zod_1.z.number().optional(),
    bookmarkNames: zod_1.z.array(zod_1.z.string().max(120)).max(300).optional(),
})
    .partial()
    .optional()
    .nullable();
const updateCompanyReportDefaultsSchema = zod_1.z.object({
    scope: zod_1.z
        .object({
        complianceStatement: zod_1.z.string().max(4000).optional(),
        independenceStatement: zod_1.z.string().max(4000).optional(),
        intendedUseStatement: zod_1.z.string().max(4000).optional(),
        scopeOfWorkDetails: zod_1.z.string().max(6000).optional(),
        valuationBasisDefinition: zod_1.z.string().max(4000).optional(),
        valuePremiseDefinition: zod_1.z.string().max(2000).optional(),
        useRestriction: zod_1.z.string().max(4000).optional(),
        externalSpecialistUse: zod_1.z.string().max(4000).optional(),
        esgConsiderations: zod_1.z.string().max(4000).optional(),
        informationSources: zod_1.z.string().max(6000).optional(),
    })
        .partial()
        .optional(),
    methodology: zod_1.z
        .object({
        assetSubjectDescription: zod_1.z.string().max(4000).optional(),
        assetDetailedDescription: zod_1.z.string().max(6000).optional(),
        methodologyRationale: zod_1.z.string().max(6000).optional(),
        costApproachDetails: zod_1.z.string().max(6000).optional(),
        salvageValueDescription: zod_1.z.string().max(4000).optional(),
        physicalDepreciationDescription: zod_1.z.string().max(6000).optional(),
        functionalObsolescenceDescription: zod_1.z.string().max(4000).optional(),
        economicObsolescenceDescription: zod_1.z.string().max(4000).optional(),
    })
        .partial()
        .optional(),
    assumptions: zod_1.z
        .object({
        generalAssumptions: zod_1.z.string().max(6000).optional(),
        specialAssumptions: zod_1.z.string().max(4000).optional(),
    })
        .partial()
        .optional(),
    customGroups: zod_1.z
        .array(zod_1.z.object({
        id: zod_1.z.string().max(120).optional(),
        title: zod_1.z.string().max(220).optional(),
    }))
        .max(30)
        .optional(),
    customSections: zod_1.z
        .array(zod_1.z.object({
        id: zod_1.z.string().max(120).optional(),
        groupId: zod_1.z.string().max(120).optional(),
        groupTitle: zod_1.z.string().max(220).optional(),
        sectionNumber: zod_1.z.string().max(40).optional(),
        title: zod_1.z.string().max(220).optional(),
        body: zod_1.z.string().max(50_000).optional(),
    }))
        .max(40)
        .optional(),
    letterhead: zod_1.z
        ...
        templateId: zod_1.z.string().max(80).optional().nullable(),
        outputFormat: zod_1.z.enum(["pdf", "pptx"]).optional().nullable(),
        coverImageDataUrl: reportDefaultsImageDataUrlSchema,
        pageImageDataUrl: reportDefaultsImageDataUrlSchema,
        landscapePageImageDataUrl: reportDefaultsImageDataUrlSchema,
        ...
        .partial()
        .optional(),
    aiTemplates: zod_1.z.array(aiTemplateSchema).max(20).optional(),
...
function newReportOnlySignatoryId() {
    return `${REPORT_ONLY_SIGNATORY_ID_PREFIX}${(0, crypto_1.randomId)()}`;
}
function isReportOnlySignatoryId(id) {
    return id.startsWith(REPORT_ONLY_SIGNATORY_ID_PREFIX);
}
function normalizeReportOnlySignatories(raw) {
    ...
    for (const item of raw) {
        ...
        if (!id || !name || seen.has(id))
            continue;
        seen.add(id);
        const signature = typeof row.signatureImageDataUrl === "string" &&
        ...
        out.push({
            ...
            membershipNo: normalizeOptionalText(typeof row.membershipNo === "string" ? row.membershipNo : null) ??
                undefined,
            signatureImageDataUrl: signature,
            createdAt: typeof row.createdAt === "string" && row.createdAt.trim()
                ? row.createdAt
...
function reportOnlySignatoryToApiRow(row) {
    ...
    return {
        ...
        signatureImageDataUrl: typeof row.signatureImageDataUrl === "string" &&
            row.signatureImageDataUrl.startsWith("data:image/")
            ? row.signatureImageDataUrl
            : "",
...
const createReportOnlySignatorySchema = zod_1.z.object({
    name: zod_1.z
        .string()
        .trim()
        .min(2)
    ...
    jobTitle: zod_1.z.string().trim().max(160).optional().or(zod_1.z.literal("")),
    membershipNo: zod_1.z.string().trim().max(80).optional().or(zod_1.z.literal("")),
});
const updateReportOnlySignatorySchema = zod_1.z.object({
    name: zod_1.z
        .string()
        .trim()
        .min(2)
...
const updateReportOnlySignatorySignatureSchema = zod_1.z.object({
    signatureImageDataUrl: zod_1.z.union([
        zod_1.z
            .string()
            .max(700_000)
            .refine((s) => s === "" || s.startsWith("data:image/"), {
...
const createCompanySchema = zod_1.z.object({
    ...
    username: zod_1.z.string().trim().min(3).max(64).optional().or(zod_1.z.literal("")),
...
function sanitizeCompanyUserJsonBody(body) {
    ...
    if (o.valuationReportMembershipNo === null)
        delete o.valuationReportMembershipNo;
    return o;
}
...
const createCompanyUserSchema = zod_1.z.object({
    username: zod_1.z.preprocess(asTrimmedString, zod_1.z.string().trim().min(3).max(64)).optional(),
    ...
    if (companyIds.length > 0) {
        ...
        const adminDocs = adminUserIds.length > 0
            ? await users
                ...
                .toArray()
            : [];
        const nameById = new Map(adminDocs.map((u) => [u._id.toString(), u.phone ?? u.username]));
...
async function listCompanyUsersForCompanyAdmin(request) {
    ...
    return {
        ...
        payload: {
            ...
            reportOnlySignatories: normalizeReportOnlySignatories(company?.reportOnlySignatories).map((row) => ({
                id: row.id,
                name: row.name,
...
async function resolveCompanyAdminReportIdentity(db, companyId, company) {
...
async function getCompanyReportDefaultsForMember(request) {
    ...
    const memberUsers = await users
        ...
        .limit(200)
        .toArray();
    const memberSignatoryRows = memberUsers
        .map((u) => {
...
async function updateCompanyReportDefaultsByCompanyAdmin(request, body) {
    const context = await (0, context_1.resolveRequestContext)(request);
    assertCompanyAdminUser(context);
    assertCsrf(request);
    ...
    await companies.updateOne({ _id: companyId }, { $set: { reportDefaults: sanitized, updatedAt: new Date() } });
    return {
        context,
        payload: {
            ok: true,
...
async function updateCompanyMemberReportSignatureByCompanyAdmin(request, body) {
    ...
    const requestedTargetOid = (0, object_id_util_1.tryParseObjectId)(parsed.data.userId);
    const targetOid = context.companyMembership.role === "company_admin"
        ? requestedTargetOid
        : context.user._id;
    if (!targetOid) {
...
async function getCurrentCompanyUserSignature(request) {
    const context = await (0, context_1.resolveRequestContext)(request);
    assertCompanyMemberUser(context);
    ...
    return {
        ...
        payload: {
            ...
            valuationReportSignatureDataUrl: typeof value === "string" && value.startsWith("data:image/") ? value : null,
        },
    };
}
async function createCompanyReportOnlySignatory(request, body) {
    const context = await (0, context_1.resolveRequestContext)(request);
    assertCompanyAdminUser(context);
    assertCsrf(request);
    ...
    await companies.updateOne({ _id: companyId }, {
        $set: {
...
async function updateCompanyReportOnlySignatory(request, signatoryId, body) {
    const context = await (0, context_1.resolveRequestContext)(request);
    assertCompanyAdminUser(context);
    assertCsrf(request);
    ...
    const updated = {
        ...
        jobTitle: parsed.data.jobTitle !== undefined
            ...
            : current.jobTitle,
        membershipNo: parsed.data.membershipNo !== undefined
...
async function updateCompanyReportOnlySignatorySignature(request, signatoryId, body) {
    const context = await (0, context_1.resolveRequestContext)(request);
    assertCompanyAdminUser(context);
    assertCsrf(request);
    const id = String(signatoryId || "").trim();
    ...
    return { context, payload: { ok: true } };
}
async function deleteCompanyReportOnlySignatory(request, signatoryId) {
    const context = await (0, context_1.resolveRequestContext)(request);
    assertCompanyAdminUser(context);
    assertCsrf(request);
    const id = String(signatoryId || "").trim();
...
async function createCompanyUserByCompanyAdmin(request, body) {
    ...
    if (existingUser) {
        ...
        await userCompanyMemberships.updateOne({ _id: existingLink._id }, {
            $set: membershipSet,
        });
        ...
        if (!existingUser.company || existingUser.company.equals(companyId)) {
            ...
            if (existingLink.role !== "company_admin") {
                userSet.role = payload.role;
            }
        }
        await users.updateOne({ _id: existingUser._id }, { $set: userSet });
        await userProfiles.updateOne({ userId: existingUser._id }, {
            $set: {
                userId: existingUser._id,
        ...
        const linkedMembership = (await userCompanyMemberships.findOne({ _id: existingLink._id })) ?? existingLink;
        await recordActivities(context.session._id, context.identityId, context.user._id, [
            {
...
async function updateCompanyUserByCompanyAdmin(request, userId, body) {
    ...
    if (parsed.data.productId !== undefined) {
        const nextProductIds = mergeProductIds(membershipProductIds(link, context.company), parsed.data.productId, context.company);
        await userCompanyMemberships.updateOne({ _id: link._id }, { $set: { productIds: nextProductIds, updatedAt: now } });
    }
...
async function deleteCompanyUserByCompanyAdmin(request, userId) {
    ...
    if (productId) {
        ...
        if (remainingProductIds.length > 0) {
            await userCompanyMemberships.updateOne({ _id: link._id }, { $set: { productIds: remainingProductIds, updatedAt: new Date() } });
            await recordActivities(context.session._id, context.identityId, context.user._id, [
                {
            ...
            return {
                context,
                payload: { success: true, productUnlinked: true },
            };
        }
    }
...
const updateCompanyBySuperAdminSchema = zod_1.z.object({
    ...
    adminPhone: zod_1.z.string().trim().min(6).max(32).refine(isValidPhoneIdentifier, {
        message: "Phone number must include country code.",
    }).optional(),

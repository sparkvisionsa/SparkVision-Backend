...
async function persistCompanyReportLetterheadImage(companyId, field, value) {
    ...
    const filename = `${field}-${Date.now()}-${(0, crypto_1.randomId)()}.${parsed.ext}`;
...
function resolveCompanyReportDefaults(stored) {
...
async function resolveCompanyAdminReportIdentity(db, companyId, company) {
    ...
    if (!adminUserId)
        return { membershipNo: null, name: null };
...
async function updateCompanyReportDefaultsByCompanyAdmin(request, body) {
    ...
    return {
        context,
        payload: { ok: true, reportDefaults: resolveCompanyReportDefaults(sanitized) },
    };
}

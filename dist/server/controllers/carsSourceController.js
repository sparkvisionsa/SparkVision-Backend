...
const mobasherAuctionsController_1 = require("./mobasherAuctionsController");
...
const SOURCE_KEYS = ["haraj", "mobasher"];
const CARS_IND_HARAJ_OPTIONS = { dataSource: "cars-ind" };
...
function buildSuggestionSearchTerms(rawQuery) {
    ...
    const canonicalRaw = (0, vehicle_name_match_1.toVehicleCanonicalKey)(rawQuery);
    const candidates = [
        rawQuery,
        canonicalRaw,
        ...(0, vehicle_name_match_1.buildVehicleAliases)(rawQuery),
        ...phraseCandidates,
        ...normalizedTokens,
    ...
    for (const candidate of candidates) {
        ...
        terms.push(trimmed);
        if (terms.length >= SUGGESTIONS_MAX_SEARCH_TERMS)
            break;
    }
    return terms.length > 0 ? terms : [rawQuery.trim()];
}
function buildSuggestionListQuery(query, search, fetchLimit = SUGGESTIONS_SOURCE_FETCH_LIMIT) {
...
function toCompactSuggestionText(value) {
    return normalizeSuggestionText(value).replace(/\s+/g, "");
}
function boundedLevenshteinDistance(a, b, maxDistance = SUGGESTION_MAX_FUZZY_DISTANCE) {
    if (a === b)
        return 0;
    if (!a || !b)
...
function toLooseSuggestionToken(value) {
    return normalizeSuggestionText(value)
        .replace(/[\u0627\u0648\u064a\u0649\u0647\u0629\u0621\u0624\u0626]/g, "")
        .replace(/[aeiouy]/g, "")
        .replace(/\s+/g, "");
}
function resolveTokenMaxDistance(a, b) {
    const minLength = Math.min(a.length, b.length);
    if (minLength >= 7)
        return SUGGESTION_MAX_TOKEN_DISTANCE;
    return 1;
}
function scoreSuggestionTokenSimilarity(queryToken, candidateToken) {
    ...
    if (normalizedCandidate.startsWith(normalizedQuery) ||
        ...
        const ratio = Math.min(normalizedQuery.length, normalizedCandidate.length) /
...
function buildSuggestionQueryProfile(queryText) {
    ...
    for (const variant of variants) {
        ...
        canonicalVariants.push(toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(variant)));
    }
    const normalizedQueryTokens = tokenizeSuggestionText(queryText);
    const queryCanonicalTokens = normalizedQueryTokens.map((token) => toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(token)));
    const queryYearTokens = normalizedQueryTokens.filter((token) => /^(19|20)\d{2}$/.test(token));
    return {
        normalizedVariants,
        compactVariants,
        canonicalVariants,
...
function extractSuggestionCandidates(source, item) {
    ...
    const modelYear = typeof item.carModelYear === "number" && Number.isFinite(item.carModelYear)
        ? String(Math.trunc(item.carModelYear))
        : toSuggestionString(item.carModelYear);
    const mileage = typeof item.mileage === "number" && Number.isFinite(item.mileage)
    ...
    if (title && title.toLowerCase() !== "untitled") {
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(title)),
    ...
    if (brand) {
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(brand)),
    ...
    if (model) {
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(model)),
    ...
    if (brand && model) {
        ...
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(combined)),
    ...
    if (city) {
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(city)),
    ...
    if (modelYear) {
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(modelYear)),
    ...
    if (mileage) {
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(mileage)),
    ...
    if (priceText) {
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(priceText)),
    ...
    if (brand && model && modelYear) {
        ...
        suggestions.push({
            ...
            canonical: toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(combinedWithYear)),
    ...
    for (const suggestion of suggestions) {
        ...
        normalizedSuggestions.push({
            ...
            canonical: suggestion.canonical ||
                toCompactSuggestionText((0, vehicle_name_match_1.toVehicleCanonicalKey)(suggestion.label)),
            tokens: suggestion.tokens ?? tokenizeSuggestionText(suggestion.label),
        });
    }
    return normalizedSuggestions;
}
async function listSourceItemsForSuggestions(source, query, fetchLimit = SUGGESTIONS_SOURCE_FETCH_LIMIT) {
    if (source === "haraj") {
        const result = await (0, harajScrapeController_1.listHarajScrapes)(query, {
            maxLimit: fetchLimit,
            ...CARS_IND_HARAJ_OPTIONS,
        });
        return result.items;
    }
    const result = await (0, mobasherAuctionsController_1.listMobasherAuctions)(query, { maxLimit: fetchLimit });
    return result.items;
}
function scoreSuggestionCandidate(candidate, profile) {
    const { normalizedVariants, compactVariants, canonicalVariants, normalizedQueryTokens, queryCanonicalTokens, queryYearTokens, } = profile;
    ...
    for (let index = 0; index < normalizedVariants.length; index += 1) {
        ...
        const compactDistance = boundedLevenshteinDistance(candidate.compact, variantCompact, SUGGESTION_MAX_FUZZY_DISTANCE);
        ...
        if (candidate.canonical && variantCanonical) {
            const canonicalDistance = boundedLevenshteinDistance(candidate.canonical, variantCanonical, 2);
    ...
    const tokenMatchScores = normalizedQueryTokens.map((token, index) => {
        ...
        if (canonicalToken && candidate.canonical) {
            ...
            else if (candidate.canonical.includes(canonicalToken) ||
    ...
    const matchedTokenCount = tokenMatchScores.filter((score) => score >= SUGGESTION_MIN_TOKEN_SIMILARITY).length;
    const strongMatchedTokenCount = tokenMatchScores.filter((score) => score >= SUGGESTION_STRONG_TOKEN_SIMILARITY).length;
    const tokenSimilarityCoverage = normalizedQueryTokens.length > 0
        ? tokenMatchScores.reduce((sum, score) => sum + score, 0) / normalizedQueryTokens.length
        : 0;
    const queryTokenCoverage = normalizedQueryTokens.length > 0
        ? matchedTokenCount / normalizedQueryTokens.length
        : 0;
    const candidateTokenCoverage = candidate.tokens.length > 0
        ? matchedTokenCount / candidate.tokens.length
        : 0;
    const candidateYearTokens = candidate.tokens.filter((token) => /^(19|20)\d{2}$/.test(token));
    const matchedYearCount = queryYearTokens.filter((yearToken) => candidateYearTokens.includes(yearToken)).length;
    if (bestVariantScore < 0 &&
        matchedTokenCount === 0 &&
        tokenSimilarityCoverage < 0.45) {
        return -1;
    }
    const fullTokenBonus = normalizedQueryTokens.length > 0 && strongMatchedTokenCount === normalizedQueryTokens.length
        ? 44
        : matchedTokenCount * 12;
    const tokenCoverageBonus = Math.round(queryTokenCoverage * 24) +
        Math.round(candidateTokenCoverage * 12) +
        Math.round(tokenSimilarityCoverage * 40);
    const orderedTokenBonus = normalizedQueryTokens.length > 1 &&
        candidate.normalized.includes(normalizedQueryTokens.join(" "))
        ? 22
        : 0;
    const yearBonus = matchedYearCount > 0 ? matchedYearCount * 14 : 0;
    const canonicalBonus = candidate.canonical &&
        queryCanonicalTokens.some((canonicalToken) => canonicalToken ? candidate.canonical.includes(canonicalToken) : false)
        ? 12
        : 0;
    ...
    return ((bestVariantScore < 0 ? 46 : bestVariantScore) +
        fullTokenBonus +
        tokenCoverageBonus +
        orderedTokenBonus +
        yearBonus +
...
function normalizeMobasherItems(items) {
    return items.map((item) => ({
        ...item,
        postDate: toEpochMillis(item.postDate ?? null),
        source: "mobasher",
    ...
    return (0, runtime_cache_1.getOrSetRuntimeCacheStaleWhileRevalidate)(cacheKey, cacheTtlMs, cacheStaleTtlMs, async () => {
        ...
        if (query.fields === "modelYears") {
            ...
            const [harajData, mobasherData] = await Promise.all([
                ...
                includeMobasher
                    ? (0, mobasherAuctionsController_1.listMobasherAuctions)({
                        ...modelYearsQuery,
                        page: 1,
                        limit: MAX_INTERNAL_LIMIT,
                        fields: "modelYears",
        ...
        if (!includeHaraj && includeMobasher) {
            const mobasherData = await (0, mobasherAuctionsController_1.listMobasherAuctions)({
                ...query,
                page,
                limit,
            }, { maxLimit: MAX_LIMIT });
            return {
                ...mobasherData,
                items: normalizeMobasherItems(mobasherData.items),
                page,
                limit,
            };
        }
        if (query.fields === "options") {
            ...
            const normalizedMobasherItems = normalizeMobasherItems(mobasherData.items);
            const mergedOptionItems = mergeSortedItems([normalizedHarajItems, normalizedMobasherItems], query.sort, limit);
            return {
                ...
                ...(countMode === "none"
                    ? {
                        hasNext: Boolean(harajData.hasNext) ||
                            Boolean(mobasherData.hasNext),
                    }
                    : {}),
            };
        }
        const hasSearch = Boolean(query.search?.trim());
        const perSourceLimit = Math.min(limit * page + (countMode === "none" ? 1 : 0), hasSearch ? MERGED_SEARCH_MAX_PER_SOURCE : MAX_INTERNAL_LIMIT);
        const [harajData, mobasherData] = await Promise.all([
            includeHaraj
                ? (0, harajScrapeController_1.listHarajScrapes)({
                    ...query,
        ...
        const mergedWindow = mergeSortedItems([normalizedHarajItems, normalizedMobasherItems], query.sort, takeCount);
...
async function listCarsSourceSearchSuggestions(query) {
    ...
    return (0, runtime_cache_1.getOrSetRuntimeCacheStaleWhileRevalidate)(cacheKey, SUGGESTIONS_CACHE_TTL_MS, SUGGESTIONS_CACHE_STALE_TTL_MS, async () => {
        ...
        for (const searchTerm of searchTerms) {
            const listQuery = buildSuggestionListQuery(query, searchTerm, SUGGESTIONS_SOURCE_FETCH_LIMIT);
            const sourceRows = await Promise.all(sources.map(async (source) => {
                const items = await listSourceItemsForSuggestions(source, listQuery, SUGGESTIONS_SOURCE_FETCH_LIMIT);
                return {
                    source,
                    items,
                };
        ...
        if (allCandidates.length < limit) {
            const fallbackQuery = buildSuggestionListQuery(query, undefined, SUGGESTIONS_FALLBACK_SOURCE_FETCH_LIMIT);
            const fallbackSourceRows = await Promise.all(sources.map(async (source) => {
                const items = await listSourceItemsForSuggestions(source, fallbackQuery, SUGGESTIONS_FALLBACK_SOURCE_FETCH_LIMIT);
                return {
                    source,
                    items,
                };
            ...
            for (const sourceRow of fallbackSourceRows) {
                for (const item of sourceRow.items) {
                    allCandidates.push(...extractSuggestionCandidates(sourceRow.source, item));
                    if (allCandidates.length >= maxRawCandidates)
                        break;
                }
                if (allCandidates.length >= maxRawCandidates)
                    break;

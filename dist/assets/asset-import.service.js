...
let AssetImportService = class AssetImportService {
    ...
    async importFile(input) {
        ...
        if (!isSupportedAssetImportFile(sanitizedFileName, sanitizedMimeType)) {
            throw new common_1.BadRequestException("نوع الملف غير مدعوم. الأنواع المقبولة: XLSX و XLSM و XLS و CSV.");
        }
    ...
    async loadWorkbook(buffer, fileName, mimeType) {
        ...
        try {
            ...
            throw new common_1.BadRequestException(`تعذر قراءة ملف الاستيراد. تحقق أن الملف ليس تالفاً وأنه محفوظ كـ XLSX أو XLSM حقيقي (وليس «صفحة ويب» بامتداد Excel).${hint}`);
        }
    }
    assertBufferMatchesExtension(buffer, extension) {
        if (extension === "xlsx" || extension === "xlsm") {
            if (!(0, asset_import_utils_1.isLikelyXlsxZipBuffer)(buffer)) {
                throw new common_1.BadRequestException(`الملف بامتداد .${extension} لكن المحتوى لا يطابق تنسيق Excel الحديث (ملف Zip). أعد الحفظ من Excel بصيغة XLSX أو XLSM صحيحة.`);
            }
        }
        if (extension === "xls") {

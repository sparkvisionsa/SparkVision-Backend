...
function normalizeArabicSearchToken(value) {
    return value
        .toLowerCase()
...
function normalizeTextSearchToken(value) {
    return normalizeArabicSearchToken(value)
        .replace(/[^a-z0-9\u0621-\u064A\s]+/gi, " ")
        .replace(/\s+/g, " ")
        .trim();
}
function toArabicCharClass(char) {
    if (char === "\u0627")
        return "[\u0627\u0623\u0625\u0622\u0671]";
    if (char === "\u064A")
        return "[\u064A\u0649\u0626]";
...
function buildArabicLoosePattern(value) {
    const normalized = normalizeArabicSearchToken(value)
        .replace(/[-_]+/g, " ")
        .replace(/\s+/g, " ")
        .trim();
    if (!normalized || !ARABIC_CHAR_REGEX.test(normalized))
        return null;
    ...
    const uniquePatterns = Array.from(new Set(variantPatterns.filter((pattern) => pattern.length > 0)));
...
function dedupeNormalized(values, limit) {
    const seen = new Set();
    const output = [];
...
function buildSmartSearchTermGroups(search, options = {}) {
    ...
    if (exact) {
        const exactTokens = dedupeNormalized(tokenizeSearch(input), maxTerms);
...
function buildSmartTextSearchQuery(search, options = {}) {
    const input = String(search ?? "").trim();
    if (!input)
        return "";
    const exact = options.exact === true;

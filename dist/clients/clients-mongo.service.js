...
function normalizeSystemData(raw) {
    ...
    for (const [productId, value] of Object.entries(raw)) {
        ...
        out[productId] = {
            ...
            formTemplateId: typeof rec.formTemplateId === "string" && rec.formTemplateId
                ? rec.formTemplateId
                : null,
            ...
            bankAccountAddress: typeof rec.bankAccountAddress === "string"
...
let ClientsMongoService = class ClientsMongoService {
    ...
    async resolveScope(request, productInput) {
        ...
        const companyId = context.company
            ? new mongoose_2.Types.ObjectId(context.company._id.toString())
            : null;
    ...
    async listClientTypes(request, query = {}) {
        ...
        if (!scope.companyId) {
            throw new common_1.BadRequestException({ message: "الرجاء اختيار شركة أولاً" });
        }
    ...
    async createClient(request, body) {
        ...
        const linkFromClientId = typeof body.linkFromClientId === "string" &&
            mongoose_2.Types.ObjectId.isValid(body.linkFromClientId)
            ? body.linkFromClientId
            : "";
        if (linkFromClientId) {
            const row = await this.clientModel
                .findOne({
                _id: new mongoose_2.Types.ObjectId(linkFromClientId),
                $and: [this.scopedClientFilter(scope, true)],
            })
                .exec();
            if (!row)

...
const logger = new common_1.Logger("TransactionsMediaService");
function transactionIdFilter(transactionId) {
    if (mongodb_1.ObjectId.isValid(transactionId)) {
        return {
            $or: [
                { transactionId: transactionId },
                { transactionId: new mongodb_1.ObjectId(transactionId) },
            ],
        };
    }
    return { transactionId };
}
function byIdAndTransaction(docId, transactionId) {
    return {
...
async function safeUnlink(filePath) {
    ...
    try {
        ...
        logger.warn(`safeUnlink: could not delete ${filePath} — ${err?.code ?? err}`);
    }
}
let TransactionsMediaService = class TransactionsMediaService {
    ...
    async listAttachments(transactionId) {
        ...
        const raw = await db
            ...
            .sort({ uploadedAt: 1 })
            .toArray();
        logger.debug(`listAttachments: found ${raw.length} docs for txId=${transactionId}`);
        if (raw.length > 0) {
            logger.debug(`listAttachments sample doc transactionId type: ${typeof raw[0].transactionId}, value: ${raw[0].transactionId}`);
        }
        return raw.map((doc) => (0, transactions_media_model_1.toAttachmentJson)(doc));
    }
    ...
    async renameAttachment(transactionId, attachmentId, name) {
        ...
        const doc = await db
            .collection(transactions_media_model_1.ATTACHMENTS_COLLECTION)
            .findOneAndUpdate(filter, { $set: { name: name.trim() } }, { returnDocument: "after" });
        if (!doc)
    ...
    async bulkDeleteAttachments(transactionId, ids) {
        ...
        const filter = { _id: { $in: objectIds }, ...txFilter };
        logger.debug(`bulkDeleteAttachments filter: ${JSON.stringify(filter)}`);
        const docs = await db
            .collection(transactions_media_model_1.ATTACHMENTS_COLLECTION)
            .find(filter)
            .toArray();
        logger.debug(`bulkDeleteAttachments: found ${docs.length} of ${ids.length} requested`);
        if (!docs.length)
            throw new common_1.NotFoundException({ message: "لم يتم العثور على مرفقات" });
        await db.collection(transactions_media_model_1.ATTACHMENTS_COLLECTION).deleteMany(filter);
    ...
    async listImages(transactionId) {
        ...
        const raw = await db
            ...
            .sort({ sortIndex: 1 })
            .toArray();
        return raw.map((doc) => (0, transactions_media_model_1.toImageJson)(doc));
    }
    ...
    async reorderImages(transactionId, order) {
        ...
        const existing = await db
            .collection(transactions_media_model_1.IMAGES_COLLECTION)
            .find({ _id: { $in: objectIds }, ...txFilter })
            .toArray();
        ...
        const updated = await db
            ...
            .sort({ sortIndex: 1 })
            .toArray();
        return updated.map((doc) => (0, transactions_media_model_1.toImageJson)(doc));
    }
    async deleteImage(transactionId, imageId) {
        ...
        const doc = await db.collection(transactions_media_model_1.IMAGES_COLLECTION).findOneAndDelete(filter);
        if (!doc)
            throw new common_1.NotFoundException({ message: "الصورة غير موجودة" });
        await safeUnlink(doc.filePath);
        const txFilter = transactionIdFilter(transactionId);
    ...
    async bulkDeleteImages(transactionId, ids) {
        ...
        const docs = await db.collection(transactions_media_model_1.IMAGES_COLLECTION).find(filter).toArray();
        ...
        await db.collection(transactions_media_model_1.IMAGES_COLLECTION).deleteMany(filter);
        await Promise.all(docs.map((d) => safeUnlink(d.filePath)));
        const remaining = await db
            .collection(transactions_media_model_1.IMAGES_COLLECTION)
            .find(txFilter)

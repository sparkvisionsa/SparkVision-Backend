...
async function toTransactionJson(db, d) {
    const evalData = { ...(0, transactions_model_1.emptyEvalData)(), ...(d.evalData ?? {}) };
    let clientName = d.clientId;
    if (d.clientId && mongodb_1.ObjectId.isValid(d.clientId)) {
        const client = await db
            .collection(clientsModule_1.CLIENTS_COLLECTION)
            .findOne({ _id: new mongodb_1.ObjectId(d.clientId) }, { projection: { name: 1 } });
        if (client?.name)
            clientName = client.name;
    }
    let templateName = null;
    if (d.templateId && mongodb_1.ObjectId.isValid(d.templateId)) {
        const tpl = await db
            .collection(clientsModule_1.FORM_TEMPLATES_COLLECTION)
            .findOne({ _id: new mongodb_1.ObjectId(d.templateId) }, { projection: { name: 1 } });
        if (tpl?.name)
...
function extractEvalData(body) {
    ...
    const extractAvailableServices = () => {
        ...
        return (0, transactions_model_1.emptyAvailableServices)();
    };
    return {
        ...
        buildingCondition: (() => {
            ...
            if (src && typeof src === "object" && !Array.isArray(src)) {
                return {
                    ...
                    completionPct: typeof src.completionPct === "number"
                        ? src.completionPct
                        : src.completionPct !== null &&
                            ...
                            : null,
                    otherText: typeof src.otherText === "string" ? src.otherText.trim() : "",
                };
            }
...
let TransactionsMongoService = class TransactionsMongoService {
    async listTransactions(companyId, inspectorId) {
        const db = await (0, mongodb_2.getMongoDb)();
        const filter = {};
        if (companyId)
            filter.companyId = companyId;
        if (inspectorId)
            filter.assignedInspectorIds = inspectorId;
        const rows = await db
    ...
    async setCompleted(id, isCompleted) {
        ...
        const row = await db
            .collection(transactions_model_1.TRANSACTIONS_COLLECTION)
            .findOneAndUpdate({ _id: new mongodb_1.ObjectId(id) }, { $set: { isCompleted, updatedAt: new Date() } }, { returnDocument: "after" });
        if (!row)
            throw new common_1.NotFoundException({ message: "المعاملة غير موجودة" });
        return toTransactionJson(db, row);
    }
    async assignInspectors(id, inspectorIds) {
        if (!mongodb_1.ObjectId.isValid(id))
            throw new common_1.NotFoundException({ message: "المعاملة غير موجودة" });
        const db = await (0, mongodb_2.getMongoDb)();
        const now = new Date();
        const row = await db
            .collection(transactions_model_1.TRANSACTIONS_COLLECTION)
            .findOneAndUpdate({ _id: new mongodb_1.ObjectId(id) }, { $set: { assignedInspectorIds: inspectorIds, updatedAt: now } }, { returnDocument: "after" });
        if (!row)
            throw new common_1.NotFoundException({ message: "المعاملة غير موجودة" });
        return toTransactionJson(db, row);
    }
    async getTransaction(id, markOpened = false) {
        if (!mongodb_1.ObjectId.isValid(id))
            throw new common_1.NotFoundException({ message: "المعاملة غير موجودة" });
        const db = await (0, mongodb_2.getMongoDb)();
        if (markOpened) {
            await db
                .collection(transactions_model_1.TRANSACTIONS_COLLECTION)
                .updateOne({ _id: new mongodb_1.ObjectId(id), isOpened: { $ne: true } }, { $set: { isOpened: true, updatedAt: new Date() } });
        }
        const row = await db
            .collection(transactions_model_1.TRANSACTIONS_COLLECTION)
    ...
    async createTransaction(body, files, meta = {}) {
        ...
        const doc = {
            ...
            templateFieldValues,
            evalData: {
                ...(0, transactions_model_1.emptyEvalData)(),
